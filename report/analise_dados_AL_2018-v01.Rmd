...
tot <- sum(RM.DA)
disc <- tot - conc
est <- binom.test(conc, tot)$estimate*100
ic <- binom.test(conc, tot)$conf.int*100
# kable(RM.DA)
...
A RM apresentou sucesso no diagnóstico de `r conc ` lesões em alça de balde e uma lesão longitudinal.
Apresentou erros de diagnóstico em um total de `r disc` classificadas erroneamente.
Das lesões com erro de classificação, 2 foram classificadas como ausentes (falso negativo). 

A taxa de acerto da RM na amostra deste estudo foi de `r format.float(est, 1)`% (IC `r format.interval(ic, 1)`%) das lesões corretamente classificadas.
...
```{r include=FALSE}
# kable(print(CreateTableOne("loc", "lca", dados), exact = T, showAllLevels = T))
```
...
1. A RM apresentou `r format.float(est, 0)`% de acerto ao identificar lesões no pré-operatório, e esta taxa é significativamente menor que 50%.
...
```{r dados recebidos, include=FALSE}
kable(dados)
```
